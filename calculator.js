...
class ChomageWizard {
    constructor() {
        ...
        // Constantes fixas do sistema suíço (não modificáveis)
        this.constants = {
            AVERAGE_WORK_DAYS: 21.7,  // Média anual para cálculo da indemnité journalière
            MONTHLY_CHILD_ALLOWANCE: 311.00  // Allocation familiale por criança
        };
        ...
        this.data = {
            ...
            isFirstMonth: true,
            controlledDays: 22  // Dias para este mês específico (padrão)
        };
        
        this.initializeEventListeners();
        this.updateCalculationMethod();
        
        // Inicializar valores do DOM após um pequeno delay
        setTimeout(() => {
    ...
    calculateDailyAllowance() {
        if (this.data.gainAssured > 0) {
            // Usar a média anual fixa para calcular indemnité journalière
            this.data.dailyAllowance = this.roundToSwissCents((this.data.gainAssured * this.data.compensationRate / 100) / this.constants.AVERAGE_WORK_DAYS);
            ...
            if (this.data.dailyAllowance <= 140 && this.data.compensationRate === 70) {
                ...
                this.data.dailyAllowance = this.roundToSwissCents((this.data.gainAssured * 80 / 100) / this.constants.AVERAGE_WORK_DAYS);
    ...
    calculateFinalResults() {
        // Pegar apenas os dias controlados que o usuário quer calcular
        const controlledDays = parseInt(document.getElementById('controlled-days')?.value || 22);
        this.data.controlledDays = controlledDays;
        
        // Garantir que temos os dados de situação pessoal mais atuais
        this.updatePersonalSituation();
    ...
    calculateLPPDeduction(dailySalary, numberOfDays) {
        ...
        return this.roundToSwissCents(Math.max(0, lppDeduction)); // S'assurer que la déduction n'est jamais négative
    }
    ...
    getLPPCalculationDetails(dailySalary, numberOfDays) {
        ...
        return {
            partieAssurable,
            explanation,
            lppDeduction: this.roundToSwissCents(Math.max(0, lppDeduction)),
    ...
    generateFinalResults() {
        ...
        // Cálculos - usar dias efetivos (controlled days - waiting period)
        const grossDailyBenefit = this.roundToSwissCents(this.data.dailyAllowance);
        ...
        const isLossInsuranceSelected = this.data.hasLossInsurance;
        ...
        resultsContainer.innerHTML = `
            ...
            <div class="detailed-calculation">
                ...
                <table class="results-table">
                    ...
                    <tbody>
                        ...
                        <tr class="subtotal">
                            <td><strong>Indemnité journalière</strong></td>
                            <td>${this.formatCurrency(this.data.gainAssured)} × ${this.data.compensationRate}% ÷ ${this.constants.AVERAGE_WORK_DAYS}</td>
                            <td class="currency">${this.formatCurrency(grossDailyBenefit)}</td>
                        </tr>
                        <tr class="subtotal">
                            <td><strong>Indemnité mensuelle brute</strong></td>
                            <td>${this.formatCurrency(grossDailyBenefit)} × ${effectiveDays} jours ${this.data.waitingPeriod > 0 ? `(${this.data.controlledDays} - ${this.data.waitingPeriod} délai)` : ''}</td>
                        ...
                        <tr class="bonus">
                            ...
                            <td>${this.data.numChildren} × (${this.constants.MONTHLY_CHILD_ALLOWANCE} ÷ ${this.constants.AVERAGE_WORK_DAYS}) × ${this.data.controlledDays} jours (total)</td>
            ...
            <div class="waiting-period-notice">
                ...
                <p><strong>Impact ce mois:</strong> Votre indemnité mensuelle brute est calculée sur ${effectiveDays} jours effectifs (${this.data.controlledDays} jours contrôlés - ${this.data.waitingPeriod} jours d'attente).</p>
                <p><em>Note: Les allocations enfants sont payées pour la totalité des jours contrôlés (${this.data.controlledDays} jours).</em></p>
            </div>
            ` : ''}

            ${!isLossInsuranceSelected ? `
            <div class="loss-insurance-notice">
                <h4>📝 Assurance Perte de Gain</h4>
                <p>Vous n'avez pas d'assurance perte de gain privée. Une cotisation obligatoire de ${this.formatCurrency(lossInsuranceDeduction)}/mois sera prélevée.</p>
            </div>
            ` : `
            <div class="loss-insurance-notice">
                <h4>📝 Assurance Perte de Gain</h4>
                <p>Vous avez une assurance perte de gain privée. Vous êtes exempté de la cotisation PCM.</p>
            </div>
            `}
        `;
    }
